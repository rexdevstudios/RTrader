    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$MEMECOIN token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $MEMECOIN token to your wallet.');
    }
  });
})();
...
(function initFlywheelDApp() {
  ...
  async function updateRealBurn() {
    try {
      ...
      const burnRes = await executeRpcWithFailover('base', 'eth_call', [{ to: '0x2222222222222222222222222222222222222222', data: deadData }, 'latest']);
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const balHex = await executeRpcWithFailover('base', 'eth_call', [{ to: '0x2222222222222222222222222222222222222222', data: calldata }, 'latest']);
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'MEMECOIN';
        }
        if (detectedWeight) {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$MEMECOIN';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$MEMECOIN';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      if ('base' === 'solana') {
        ...
        executeLink.href = 'https://app.doppler.lol/tokens/base/0x2222222222222222222222222222222222222222';
      } else {
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $MEMECOIN';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $MEMECOIN in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $MEMECOIN in Wallet...';
...
(function initTerminal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      try {
        const balHex = await executeRpcWithFailover('base', 'eth_getBalance', ['0x2222222222222222222222222222222222222222', 'latest']);
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to MEMECOIN node.</div>';
    } else {
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      const ethBalHex = await executeRpcWithFailover('base', 'eth_getBalance', ['0x2222222222222222222222222222222222222222', 'latest']);
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $MEMECOIN';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $MEMECOIN';
    }
  }
  updateRealTreasury();
  ...
  btn.addEventListener('click', () => {
    ...
    img.onload = () => {
      try {
        ...
        a.download = 'memecoin-banner.png';
