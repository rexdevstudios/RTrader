...
(function initCanvas() {
  ...
  if (visualEffect === 'matrix-rain') {
    ...
    function animateGrid() {
      ctx.clearRect(0, 0, width, height);
      ctx.strokeStyle = 'rgba(129, 245, 61, 0.45)';
      ctx.lineWidth = 0.6;
    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$AGENTAI token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $AGENTAI token to your wallet.');
    }
  });
})();
...
(function initFlywheelDApp() {
  ...
  async function updateRealBurn() {
    try {
      ...
      const burnRes = await executeRpcWithFailover('base', 'eth_call', [{ to: '0x1111111111111111111111111111111111111111', data: deadData }, 'latest']);
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const balHex = await executeRpcWithFailover('base', 'eth_call', [{ to: '0x1111111111111111111111111111111111111111', data: calldata }, 'latest']);
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'AGENTAI';
        }
        if (detectedWeight) {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$AGENTAI';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$AGENTAI';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      if ('base' === 'solana') {
        ...
        executeLink.href = 'https://app.doppler.lol/tokens/base/0x1111111111111111111111111111111111111111';
      } else {
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $AGENTAI';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $AGENTAI in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $AGENTAI in Wallet...';
...
(function initTerminal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      try {
        const balHex = await executeRpcWithFailover('base', 'eth_getBalance', ['0x1111111111111111111111111111111111111111', 'latest']);
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to AGENTAI node.</div>';
    } else {
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      const ethBalHex = await executeRpcWithFailover('base', 'eth_getBalance', ['0x1111111111111111111111111111111111111111', 'latest']);
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $AGENTAI';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $AGENTAI';
    }
  }
  updateRealTreasury();
