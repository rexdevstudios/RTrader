    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$SOLRPC token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $SOLRPC token to your wallet.');
    }
  });
})();
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'SOLRPC';
        }
        if (detectedWeight) {
...
(function initSwapWidget() {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$SOLRPC';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$SOLRPC';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $SOLRPC';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $SOLRPC in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $SOLRPC in Wallet...';
...
(function initTerminal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to SOLRPC node.</div>';
    } else {
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $SOLRPC';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $SOLRPC';
    }
  }
  updateRealTreasury();
