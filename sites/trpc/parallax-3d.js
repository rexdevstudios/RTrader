...
(function initCanvas() {
  ...
  if (visualEffect === 'matrix-rain') {
    ...
    function animateGrid() {
      ctx.clearRect(0, 0, width, height);
      ctx.strokeStyle = 'rgba(64, 245, 61, 0.45)';
      ctx.lineWidth = 0.6;
    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$TRPC token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $TRPC token to your wallet.');
    }
  });
})();
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'TRPC';
        }
        if (detectedWeight) {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$TRPC';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$TRPC';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      if ('base' === 'solana') {
        ...
        executeLink.href = 'https://app.doppler.lol/tokens/base/0x7CE19E4F978009EB644c27946B47221b824C0bA3';
      } else {
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $TRPC';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $TRPC in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $TRPC in Wallet...';
...
(function initTerminal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to TRPC node.</div>';
    } else {
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $TRPC';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $TRPC';
    }
  }
  updateRealTreasury();
