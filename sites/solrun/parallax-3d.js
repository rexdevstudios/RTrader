    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$SOLRUN token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $SOLRUN token to your wallet.');
    }
  });
})();
...
(function initFlywheelDApp() {
  ...
  async function updateRealBurn() {
    try {
      ...
      const burnRes = await executeRpcWithFailover('solana', 'eth_call', [{ to: '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz', data: deadData }, 'latest']);
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const balHex = await executeRpcWithFailover('solana', 'eth_call', [{ to: '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz', data: calldata }, 'latest']);
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'SOLRUN';
        }
        if (detectedWeight) {
...
(function initSwapWidget() {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$SOLRUN';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$SOLRUN';
  ...
  async function fetchDopplerQuote(val) {
    ...
    const sellToken = isBuy ? '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE' : '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
    const buyToken = isBuy ? '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz' : '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      if ('solana' === 'solana') {
        executeLink.href = 'https://pump.fun/4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
      } else if ('solana' === 'robinhood') {
        executeLink.href = 'https://pons.fun/token/4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
      } else if ('solana' === 'clanker') {
        executeLink.href = 'https://clanker.world/clanker/4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
      } else if (false) {
        executeLink.href = 'https://app.doppler.lol/tokens/base/4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
      } else {
        const amtStr = val > 0 ? String(val) : '0.05';
        executeLink.href = 'https://app.uniswap.org/swap?chain=base&inputCurrency=NATIVE&outputCurrency=4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz&exactAmount=' + encodeURIComponent(amtStr);
      }
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $SOLRUN';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            const allowanceHex = await window.ethereum.request({
              method: 'eth_call',
              params: [{ to: '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz', data: allowanceCallData }, 'latest'],
            });

            const currentAllowance = BigInt(allowanceHex || '0x0');
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $SOLRUN in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $SOLRUN in Wallet...';
              ...
              const approveTxHash = await window.ethereum.request({
                ...
                params: [{
                  from: userAddr,
                  to: '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz',
                  data: approveData,
                }],
              });
...
function launchConfetti() {
  ...
  const colors = ['#06b6d4', '#f59e0b', '#10b981', '#38bdf8', '#fbbf24', '#f43f5e'];
...
(function initTerminal() {
    ...
    try {
      if ('solana' === 'base' || 'solana' === 'clanker') {
        ...
        const liveEvents = [
          ...
          '<span class="text-emerald-400">[ON-CHAIN]</span> Token Contract: <span class="font-mono text-white">4znm13bnfx...U2BJzz</span> • Verified on Basescan',
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      appendLog('<span class="text-cyan-400">[CONTRACT]</span> <span class="font-mono text-white">4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz</span><br/>' +
        '<a href="https://basescan.org/token/4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz" target="_blank" class="text-brandPrimary hover:underline">View on Basescan ↗</a>');
    } else if (cmd === '/buyback') {
      ...
      try {
        const balHex = await executeRpcWithFailover('solana', 'eth_getBalance', ['4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz', 'latest']);
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to SOLRUN node.</div>';
    } else {
      appendLog('<span class="text-brandPrimary">[AI AGENT]</span> ' +
        'Contract <span class="font-mono text-white">4znm13bnfx...</span> is active on Base Mainnet. Type /help for on-chain commands.');
    }
  });
})();
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      const ethBalHex = await executeRpcWithFailover('solana', 'eth_getBalance', ['4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz', 'latest']);
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $SOLRUN';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $SOLRUN';
    }
  }
  updateRealTreasury();
...
(function initLpVerification() {
  ...
  const ca = '4znm13bnfx9cj8V8TSXSH7FbD3anij5RF8QQiAU2BJzz';
  const chain = 'solana';
