    ...
    if (window.ethereum) {
      try {
        ...
        if (wasAdded) {
          ...
          if (status) {
            status.innerText = '$SOLMEME token registered to wallet.';
      ...
      alert('Open in a Web3-compatible browser (MetaMask, Rabby, or Coinbase Wallet) to register $SOLMEME token to your wallet.');
    }
  });
})();
...
(function initFlywheelDApp() {
  ...
  async function updateRealBurn() {
    try {
      ...
      const burnRes = await executeRpcWithFailover('solana', 'eth_call', [{ to: '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', data: deadData }, 'latest']);
  ...
  if (checkWalletBtn && checkResultBox) {
    checkWalletBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const balHex = await executeRpcWithFailover('solana', 'eth_call', [{ to: '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', data: calldata }, 'latest']);
        ...
        if (detectedHoldings) {
          detectedHoldings.innerText = humanTokens.toLocaleString() + ' $' + 'SOLMEME';
        }
        if (detectedWeight) {
...
(function initSwapWidget() {
  ...
  function setMode(mode) {
    ...
    if (tabBuy && tabSell) {
      if (mode === 'buy') {
        ...
        if (outputCurrency) outputCurrency.innerText = '$SOLMEME';
        ethInput.value = '0.05';
      } else {
        ...
        if (inputCurrency) inputCurrency.innerText = '$SOLMEME';
  ...
  async function fetchDopplerQuote(val) {
    ...
    const sellToken = isBuy ? '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE' : '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU';
    const buyToken = isBuy ? '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU' : '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
  ...
  async function updateOutput() {
    ...
    if (executeLink) {
      if ('solana' === 'solana') {
        executeLink.href = 'https://pump.fun/7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU';
      } else if ('solana' === 'robinhood') {
        executeLink.href = 'https://pons.fun/token/7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU';
      } else if ('solana' === 'clanker') {
        executeLink.href = 'https://clanker.world/clanker/7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU';
      } else if (false) {
        executeLink.href = 'https://bankr.bot/terminal/trade?in=WETH&chain=base&out=7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU';
      } else {
        const amtStr = val > 0 ? String(val) : '0.05';
        executeLink.href = 'https://app.uniswap.org/swap?chain=base&inputCurrency=NATIVE&outputCurrency=7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU&exactAmount=' + encodeURIComponent(amtStr);
      }
      ...
      if (!userAddr) {
        executeLink.innerText = 'Connect Wallet to ' + (currentMode === 'buy' ? 'Buy' : 'Sell') + ' $SOLMEME';
      } else {
        ...
        try {
          ...
          if (currentMode === 'sell') {
            ...
            const allowanceHex = await window.ethereum.request({
              method: 'eth_call',
              params: [{ to: '7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', data: allowanceCallData }, 'latest'],
            });

            const currentAllowance = BigInt(allowanceHex || '0x0');
            if (currentAllowance < sellAmountWei) {
              if (statusBox) {
                statusBox.innerHTML = '<span class="text-cyan-400 animate-pulse">✍️ Step 1/2: Please approve $SOLMEME in wallet...</span>';
              }
              executeLink.innerText = '✍️ Approve $SOLMEME in Wallet...';
...
(function initTerminal() {
    ...
    try {
      if ('solana' === 'base' || 'solana' === 'clanker') {
        ...
        const liveEvents = [
          ...
          '<span class="text-emerald-400">[ON-CHAIN]</span> Token Contract: <span class="font-mono text-white">7xKXtg2CW8...osgAsU</span> • Verified on Basescan',
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (cmd === '/help') {
      ...
      appendLog('<span class="text-cyan-400">[CONTRACT]</span> <span class="font-mono text-white">7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU</span><br/>' +
        '<a href="https://basescan.org/token/7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU" target="_blank" class="text-brandPrimary hover:underline">View on Basescan ↗</a>');
    } else if (cmd === '/buyback') {
      ...
      try {
        const balHex = await executeRpcWithFailover('solana', 'eth_getBalance', ['7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', 'latest']);
      ...
      logs.innerHTML = '<div class="text-emerald-400">[TERMINAL CLEARED] Connected to SOLMEME node.</div>';
    } else {
      appendLog('<span class="text-brandPrimary">[AI AGENT]</span> ' +
        'Contract <span class="font-mono text-white">7xKXtg2CW8...</span> is active on Base Mainnet. Type /help for on-chain commands.');
    }
  });
})();
...
(function initTreasuryHUD() {
  ...
  async function updateRealTreasury() {
    try {
      ...
      const ethBalHex = await executeRpcWithFailover('solana', 'eth_getBalance', ['7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', 'latest']);
      ...
      if (burnElem) burnElem.innerText = burnedVal.toLocaleString() + ' $SOLMEME';
    } catch (e) {
      if (reserveElem) reserveElem.innerText = '$0';
      if (ethElem) ethElem.innerText = '(0.00 ETH)';
      if (burnElem) burnElem.innerText = '0 $SOLMEME';
    }
  }
  updateRealTreasury();
