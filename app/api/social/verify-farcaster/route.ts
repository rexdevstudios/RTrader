...
export async function POST(req: NextRequest) {
  try {
    ...
    const { walletAddress, fid, username, followersCount, custodyAddress } = body;
    ...
    const verification = FarcasterLensService.verifyFarcasterAccount({
      walletAddress,
      fid: Number(fid),
      username,
    ...
    if (!verification.isValid) {
      return NextResponse.json(
        { success: false, error: verification.reason || 'FARCASTER_VERIFICATION_FAILED' },
        { status: 400 }
      );
    }
